...
class Update_1 extends StatefulWidget {
  const Update_1({super.key});

  @override
  State<Update_1> createState() => _Update_1State();
}

class _Update_1State extends State<Update_1> {
  final FirebaseFirestore firestore = FirebaseFirestore.instance;
  final DocumentReference docref = FirebaseFirestore.instance.collection('Data_1').doc('L75inM15ljNNM6ybWOVW');
  ...
  void fetchUserData() async {
    try{
      ...
      if(documentSnapshot.exists){
        setState(() {
          Fname.text = documentSnapshot['First_Name'] ?? '';
          Mname.text = documentSnapshot['Middle_Name'] ?? '';
          Lname.text = documentSnapshot['Last_Name'] ?? '';
          email.text = documentSnapshot['Email'] ?? '';
          Mob_No.text = documentSnapshot['Mobile_No'] ?? '';
          _SelectedGender = documentSnapshot['Gender'] ?? '';
          SelecetedCountry = documentSnapshot['Country'] ?? '';
          SelectedHobbies = List.from(documentSnapshot['Hobbies'] ?? []);
          Timestamp dateTimestamp = documentSnapshot['birthDate'];
          selected_date = dateTimestamp.toDate();
          bio.text = documentSnapshot['Bio'] ?? '';
          Uname.text = documentSnapshot['Username'] ?? '';
          Pass.text = documentSnapshot['Password'] ?? '';
        });
      }
    }
    catch(e){
                    ...
                    child: Row(
                      children: [
                        ...
                        Checkbox(
                          ...
                          value: SelectedHobbies.contains('coding'),
                        ...
                        Checkbox(
                          checkColor: Colors.white,
                          activeColor: Colors.black,
                          value: SelectedHobbies.contains('games'),
                          onChanged: (bool? value) {
                            setState(() {
                              if(value!){
                                SelectedHobbies.add('games');
                              }
                              else
                                {
                                  SelectedHobbies.remove('games');
                                }
                            });
                          },
                          // onChanged: (value){},
                          ...
                          await docref.update(
                            {
                              ...
                              'birthDate' : Timestamp.fromDate(selected_date),
                              'Bio' : BIO,
                              'Username' : UName,
                              'Password' : PASS,
